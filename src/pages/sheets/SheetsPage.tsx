...
import {
  useSheetPageProtection,
  blockSheetContextMenu,
} from "../../features/sheets/hooks/useSheetPageProtection";
import { SheetProtectedPreview } from "../../features/sheets/components/SheetProtectedPreview";
import { PrintableSheetPreview } from "../../features/sheets/components/PrintableSheetPreview";
import { triggerSheetPrint } from "../../features/sheets/utils/triggerSheetPrint";
...
export function SheetsPage() {
  ...
  const clearPrintParam = () => {
    if (!searchParams.has("print")) return;
    const next = new URLSearchParams(searchParams);
    next.delete("print");
  ...
  /** Print only when opened from “Print sheet” after creating a request (?print=1). */
  useEffect(() => {
    if (!shouldAutoPrint || printedRef.current) return;
    if (!sheet || !document) return;
    printedRef.current = true;
    triggerSheetPrint(900, {
      widthMm: document.canvasWidthMm,
      heightMm: document.canvasHeightMm,
      documentTitle: sheet.name,
    });
    clearPrintParam();
  }, [shouldAutoPrint, sheet, document]);

  return (
    <div className="space-y-6" onContextMenu={blockSheetContextMenu}>
      {sheet && document && (
        <div id="sheet-print-root" className="hidden">
          <PrintableSheetPreview document={document} fieldData={sheet.fieldData} />
        </div>
      )}
      ...
      <div className="print:hidden space-y-6">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">
            To print, use <span className="font-medium">Print sheet</span> right after creating a sheet request on the
            dashboard. Print and copy shortcuts are disabled on this page.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          ...
          {templatesLoading ? (
            ...
            <FormSelect
              ...
              onValueChange={(v) => handleTemplateChange(v === "__none__" ? "" : v)}
              options={[
                { value: "__none__", label: "Select a template" },
                ...templates.map((t) => ({
                  value: t.id,
                  label: templateOptionLabel(t.title, t.id),
                })),
              ]}
              className="max-w-md"
            />
        ...
        {templateId && !showSheetLoader && !isError && pageData && (
          <>
            ...
            {sheet && document && sheet.templateId === templateId && (
              ...
              >
                <SheetProtectedPreview
                  watermarkLabel={sheet.name}
                  vendorName={user?.name}
                  className="flex min-h-[480px] flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm lg:col-span-2"
                >
